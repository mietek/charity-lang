...
def inboard: PEG * list(PEG) -> bool
    = (z,L) => {| nil: () => false
                | cons: (x,v) => PEG_eq(z,x) or v
                |} L.

def new_board: ((PEG * PEG) * PEG) * list(PEG)
                        -> list(PEG)
    = (((x,y),z),L) => cons(z,filter{ v => not (PEG_eq(v,x) or PEG_eq(v,y))
                                    } L
                           ).
...
def depend: (PEG * PEG) * (PEG * PEG) -> bool
    =  (x,y) => {ss(v) => PEG_eq(v,p0 y) or PEG_eq(v,p1 y)
                |ff => false
                } jump x.
...
def peg_filter: (PEG * PEG) * list(PEG * PEG) -> bool
    = ( x, cons(y,_)) => PEG_geq(p0 x,p0 y) or dependent(x,y)
    | _ => true.
