...
def filter[: pred: A -> bool :] #_: list(A) -> list(A)
    =  nil  => nil
    |  cons(a,Ls) => pred a; { true => cons(a, filter[: pred :] Ls)
                             | false => filt Ls }.
...
def SFfind[: pass: A -> SF(B) :] #_ : list(A) -> list(B)
    =  nil  => ff
...
def dependent
    =  x,y => depend(x,y) || depend(y,x).

def peg_filter
    = (sx,ex), lst => lst ;  { nil => true
                             | cons((sy,ey),_) => (PEG_num sx >= PEG_num sy) || dependent(x,y)
                             }.
